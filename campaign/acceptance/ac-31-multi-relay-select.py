#!/usr/bin/env python
"""AC-31 多中继择优 — 验收骨架（三态结构）。
source: SRS §8.3 条目 31（v0.5 新增）；refs: FR-10.1/10.3、C8。
阈值（逐字自 §8.3 正文，进代码常量）：3 中继（3/15/40 Mbps）≤ 15 s 短探选最快；二次命中速率表"""
import sys

ENV_NEED = "3 个中继节点限速构造环境"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：3 中继（3/15/40 Mbps）≤ 15 s 短探选最快；二次命中速率表"""
