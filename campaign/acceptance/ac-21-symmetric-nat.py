#!/usr/bin/env python
"""AC-21 对称型 NAT 双端 — 验收骨架（三态结构）。
source: SRS §8.3 条目 21（C5 新增）；refs: §4.1.2。
阈值（逐字自 §8.3 正文，进代码常量）：端口预测生效或正确降级；不得无限重试/卡死"""
import sys

ENV_NEED = "双端对称型 NAT 环境"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：端口预测生效或正确降级；不得无限重试/卡死"""
