#!/usr/bin/env python
"""AC-35 控制帧冗余双发 — 验收骨架（三态结构）。
source: SRS §8.3 条目 35（v0.5 新增）；refs: FR-8。
阈值（逐字自 §8.3 正文，进代码常量）：单路径控制帧被丢弃时冗余副本正常推进不卡死"""
import sys

ENV_NEED = "可丢弃指定控制帧的构造环境"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：单路径控制帧被丢弃时冗余副本正常推进不卡死"""
