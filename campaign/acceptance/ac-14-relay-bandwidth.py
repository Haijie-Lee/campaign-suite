#!/usr/bin/env python
"""AC-14 服务端带宽占比 — 验收骨架（三态结构）。
source: SRS §8.3 条目 14（C5 新增）；refs: NFR-2.6。
阈值（逐字自 §8.3 正文，进代码常量）：中继路径承载数据量 ≤ 全部传输量 15%"""
import sys

ENV_NEED = "同 AC-13 测试矩阵"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：中继路径承载数据量 ≤ 全部传输量 15%"""
