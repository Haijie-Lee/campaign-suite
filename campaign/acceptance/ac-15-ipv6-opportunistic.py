#!/usr/bin/env python
"""AC-15 IPv6 直连 — 验收骨架（三态结构）。
source: SRS §8.3 条目 15（C5 新增）；refs: C6、FR-8.13。
阈值（逐字自 §8.3 正文，进代码常量）：当且仅当短探测可达才进候选；不可达静默跳过"""
import sys

ENV_NEED = "可控 IPv6 可达/不可达双环境"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：当且仅当短探测可达才进候选；不可达静默跳过"""
