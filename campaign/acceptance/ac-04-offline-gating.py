#!/usr/bin/env python
"""AC-04 接收端离线门控 — 验收骨架（三态结构）。
source: SRS §8.3 条目 4（基础 12 条）；refs: FR-3。
阈值（逐字自 §8.3 正文，进代码常量）：15 s 置灰 / 409 RECEIVER_OFFLINE"""
import sys
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：15 s 置灰 / 409 RECEIVER_OFFLINE"""
