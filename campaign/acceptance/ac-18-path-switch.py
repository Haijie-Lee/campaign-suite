#!/usr/bin/env python
"""AC-18 路径切换无感 — 验收骨架（三态结构）。
source: SRS §8.3 条目 18（C5 新增）；refs: FR-8。
阈值（逐字自 §8.3 正文，进代码常量）：切中继会话不中断、已确认分片不重传、哈希一致"""
import sys

ENV_NEED = "传输中强制断开直连的构造环境"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：切中继会话不中断、已确认分片不重传、哈希一致"""
