#!/usr/bin/env python
"""AC-13 直连成功率 — 验收骨架（三态结构）。
source: SRS §8.3 条目 13（C5 新增）；refs: NFR-2.5。
阈值（逐字自 §8.3 正文，进代码常量）：直连成功率 ≥ 85%（≥30 组真实网络组合，按 NAT 类型分组）"""
import sys

ENV_NEED = "不少于 30 组真实网络组合（家庭宽带 × 运营商 × 有无 IPv6 × NAT 类型）"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：直连成功率 ≥ 85%（≥30 组真实网络组合，按 NAT 类型分组）"""
