#!/usr/bin/env python
"""AC-02 投递 5 GB 单文件，中途手动断网 60 s，恢复后从断点续传，最终 BLAKE3 校验一致 — 验收骨架（三态结构）。
source: SRS §8.3 条目 2（基础 12 条）；refs: FR-6。
阈值（逐字自 §8.3 正文，进代码常量）：5 GB / 60 s"""
import sys
