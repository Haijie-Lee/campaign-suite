#!/usr/bin/env python
"""AC-27 NAT64 / DNS64 环境 — 验收骨架（三态结构）。
source: SRS §8.3 条目 27（v0.3.1 新增）；refs: §4.1.8。
阈值（逐字自 §8.3 正文，进代码常量）：NAT64 环境可连可投；无硬编码 IPv4 字面量失效"""
import sys

ENV_NEED = "仅 IPv6 出口（NAT64）网络"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：NAT64 环境可连可投；无硬编码 IPv4 字面量失效"""
