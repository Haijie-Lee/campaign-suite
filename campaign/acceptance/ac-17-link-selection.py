#!/usr/bin/env python
"""AC-17 链路择优正确性 — 验收骨架（三态结构）。
source: SRS §8.3 条目 17（C5 新增）；refs: §4.1.4。
阈值（逐字自 §8.3 正文，进代码常量）：直连 3 Mbps vs 中继 50 Mbps 必须选中继"""
import sys

ENV_NEED = "限速构造环境（直连 3 Mbps、中继 50 Mbps）"
...
def probe():
    """探针 + 断言：环境与实现就绪后执行。阈值：直连 3 Mbps vs 中继 50 Mbps 必须选中继"""
